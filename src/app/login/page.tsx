...
function Login() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    const phone: string = "+998" + loginPhone;
    const res: unknown = await authStore.login({ phone, password: loginPassword } as LogInType);
                  ...
                  <form onSubmit={handleLogin} className="flex flex-col gap-4 w-full">
                    <div className="flex gap-2 flex-col">
                      ...
                      <PatternFormat
                        ...
                        required
                        value={loginPhone}
                        onValueChange={(values: { value: string; formattedValue: string; floatValue?: number }) => setLoginPhone(values.value)} // faqat raqamlarni oladi: 901234567
                        format="+998 ## ### ## ##"
                        mask=" "
                      />
                    </div>
